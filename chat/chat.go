...
import (
	...
	"github.com/Ho-yeong/WebChat/utils"
...
func (c *Chat) Handler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Fatal("Erro on websocket connection: ", err.Error())
	}
...
func (c *Chat) add(user *User) {
	if _, ok := c.users[user.Username]; !ok {
		...
		log.Printf("Added user: %s, Total: %d", user.Username, len(c.users))
	}
}
...
func (c *Chat) disconnect(user *User) {
	if _, ok := c.users[user.Username]; ok {
		...
		log.Printf("User left: %s, Total: %d", user.Username, len(c.users))
	}
}
